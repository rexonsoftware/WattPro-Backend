...
async function getServiceContracts(req, res){
  const { id = ''} = req.query;  //Get Id
    try{
      if (id == "") return res.status(400).json({code: 400, status: 'Validation Error', message: "Kunden Nr ist erforderlich" });
...
const addServiceContracts = async (req, res, next) => {
  const { installation_id, service_id, bezeichnung = null, wartungsdatum, name_sachkundiger = null} = req.body;
...
const updateServiceContracts = async (req, res, next) => {
  const { id, installation_id = '', service_id = '', bezeichnung = '', wartungsdatum = '', name_sachkundiger = 0} = req.body;
  const user = { ID : 1 };    // req.user;
  // Validate input using the imported validateRoles array
  if (id == "" || id === undefined) return res.status(400).json({code: 400, status: 'Validation Error', message: "Service Contract Nr ist erforderlich" });
  const errors = validationResult(req);
  if (!errors.isEmpty()) return res.status(400).json({code: 400, status: 'Validation Error', message: errors.array() });
  try {
    const serviceContract = await ServiceContractModel.findByPk(id);
    if (!serviceContract) return res.status(404).json({code: 404, status:'Error', message: 'Objekt not found'  });
    if(installation_id != ""){ serviceContract.installation_id = installation_id; }
    if(service_id != ""){ serviceContract.contract_id = service_id; }
...
const deleteServiceContracts = async (req, res, next) => {
  ...
  try {
    const serviceContract = await ServiceContractModel.findByPk(id);
    if (!serviceContract) return res.status(404).json({code: 404, status:'Error', message: 'Service Contract not found'  });
